...
const data: ChartDataPoint[] = [
  { ts: BASE, value: 5 },
  { ts: BASE + HALF_HOUR, value: 10 },
  { ts: BASE + 2 * HALF_HOUR, value: 2 },
  { ts: BASE + 3 * HALF_HOUR, value: 8 },
];

const fullRange: TimeRange = { fromTs: BASE, toTs: BASE + 4 * HALF_HOUR };

describe('computeMinMax', () => {
  it('finds standard min and max', () => {
    const result = computeMinMax(data, fullRange, true);
    expect(result.min).toEqual({ ts: BASE + 2 * HALF_HOUR, value: 2 });
    expect(result.max).toEqual({ ts: BASE + HALF_HOUR, value: 10 });
  });
  ...
  it('returns nulls for empty range', () => {
    const emptyRange: TimeRange = { fromTs: BASE + 100 * HALF_HOUR, toTs: BASE + 200 * HALF_HOUR };
  ...
  it('picks within sub-range only', () => {
    ...
    const subRange: TimeRange = { fromTs: BASE + HALF_HOUR, toTs: BASE + 3 * HALF_HOUR };
