...
describe('computePriceStats', () => {
  it('computes min, max, total, count over full range', () => {
    const range: TimeRange = { fromTs: BASE, toTs: BASE + 4 * HALF_HOUR_MS };
    const result = computePriceStats(points, range);

    expect(result.count).toBe(4);
    expect(result.min).toEqual({ price: 10, ts: BASE + 2 * HALF_HOUR_MS });
    expect(result.max).toEqual({ price: 30, ts: BASE + 3 * HALF_HOUR_MS });
    expect(result.total).toBe(20 + 25 + 10 + 30);
  });

  it('respects sub-range via binary search boundaries', () => {
    const range: TimeRange = { fromTs: BASE + HALF_HOUR_MS, toTs: BASE + 3 * HALF_HOUR_MS };
    const result = computePriceStats(points, range);

    expect(result.count).toBe(2);
    expect(result.min).toEqual({ price: 10, ts: BASE + 2 * HALF_HOUR_MS });
    expect(result.max).toEqual({ price: 25, ts: BASE + HALF_HOUR_MS });
  ...
  it('handles negative prices', () => {
    ...
    const range: TimeRange = { fromTs: BASE, toTs: BASE + 2 * HALF_HOUR_MS };
    const result = computePriceStats(data, range);

    expect(result.min).toEqual({ price: -10, ts: BASE + HALF_HOUR_MS });
